                               ...
                               char **key, int *key_size)
{
    ...
    switch (pld->object_type)
    {
        ...
        PrivateKey *private_key = (PrivateKey *)pld->object;
        block = private_key->key_block;
        break;
...
int kmip_bio_locate_key_with_context(KMIP *ctx, BIO *bio,
                                  ...
                                  char **kmip_id)
{
    if(ctx == NULL || bio == NULL || uuid == NULL || uuid_size <= 0 || namespace == NULL || namespace_size <= 0)
    {
        return(KMIP_ARG_INVALID);
    }
    ...
    asi.application_namespace = &ns;
    asi.application_data = &id;

    Attribute a;
    a.type = KMIP_ATTR_APPLICATION_SPECIFIC_INFORMATION;
    a.index = 0;
    a.value = &asi;
...
int kmip_bio_locate_all_with_context(KMIP *ctx, BIO *bio, char ***kmip_id_list, int* kmip_id_count)
{
    ...
    if (pld->id_count != 0)
    {
        *kmip_id_count = pld->id_count;
        char **tmpList = (char **) ctx->calloc_func(ctx->state,(size_t)*kmip_id_count,sizeof(char*));
        for (int i = 0; i < *kmip_id_count; i++)
        {
